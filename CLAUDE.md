...
- **UI** — overview, integrations catalog (`/integrations`), connect flow (`/connect/[provider]`), connections list + detail (`/connections`, `/connections/[id]`), asset detail with cross-links (`/assets/[id]`), Infra domains (`/domains`). Global asset search is a right-side slide-over in the topbar (`command-search.tsx`, opens on `/`) — portaled to `document.body` so the topbar's `backdrop-blur` can't clip it.
